...
# source("~/BASP/R analyses/Baseball Data/get_savant_pitches_data.R")
source("D:/Files/BASP/R analyses/Baseball Data/Data Files/get_savant_pitches_data.R")
...
num_days <- (today() - most_recent_day %>% ymd()) %>% as.numeric()

every.1.days <- seq(1, num_days, 1)

back_to <- c((lubridate::today()) - every.1.days)
...
# sc_2016.07.17 <- tbl(statcast_db, "sc_all_comb_distinct") %>% filter(game_date == "2016-07-17")
# sc_2016.07.17_df <- sc_2016.07.17 %>% collect()
...
statcast_update_comb_distinct_clean <-
    ...
    mutate(
        ...
        release_spin_rate = case_when(
            release_spin_rate == "null" ~ NA_character_ , 
        ...
        launch_speed_angle = case_when(
            launch_speed_angle == "null" ~ NA_character_ , 
        ...
        inning_topbot = case_when(
            inning_topbot == "Bot" ~ "bot",
    ...
    mutate(
        horiz_angle_new = case_when(
            ...
            (sign(hc_y.new.1) == 1 | sign(hc_y.new.1) == 0 | is.na(hc_y.new.1)) ~
            horiz_angle + 0,
