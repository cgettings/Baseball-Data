...
statcast_data <- 
    ...
    filter(type == "X" & bb_type == "fly_ball" & !is.na(hit_distance_sc) & game_type == "R") %>%
...
statcast_data %>% 
    ...
    summarise(
        n = n(),
        distance = mean(hit_distance_sc),
        sd = sd(hit_distance_sc),
        se = se(hit_distance_sc)
        ) %>% 
...
fit.1 <- lmer(hit_distance_sc ~ 1 + (1 | game_year), statcast_data)
